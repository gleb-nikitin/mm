...
- **R1 session linkage index** (2026-04-28): schema v12 adds `session_index` and `session_message_links` so imported Claude/Codex/Gemini sessions can link to ac participants across all sessions, while `/active` keeps its legacy active-only resolution. Importers now vendor-prefix `raw_events.external_id`; `scripts/backfill-r1.ts` migrates existing DBs in place.
