 ...
 * stores `external_id` as `claude:<session_id>` to avoid cross-vendor
...
async function main() {
  ...
  const upsertImportState = flags.dryRun ? null : db.prepare(`INSERT INTO import_state
    ...
    VALUES (?, ?, CURRENT_TIMESTAMP, 'claude', ?, ?, ?, ?, ?, ?)
  ...
  for (const { path: filePath, mtimeMs } of candidateFiles) {
    ...
    for (const session of sessions) {
      ...
      const namespacedExternalId = `claude:${session.sessionId}`;
      ...
      if (res === 'inserted') {
        ...
        updated++;
        console.log(`  ↻ ${title}  →  raw_events (updated, chunks reset)`);
      } else {
        duplicate++;
      }
    }
